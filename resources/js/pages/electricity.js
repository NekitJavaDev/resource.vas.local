...
var lastConsumptionUrl = base_url + 'meters/' + meterId + '/last_electricity_consumption';
...
/**
 * Запрос данные при загрузке страницы
 */
...
function getChartData(consumptionObject) {5
    const plotData = [];
 ...
 */
// $("#getFreshData").click(refreshLastConsumption) //так было
// $("#getFreshData").click(refreshLastConsumption).click(refreshData);
...
$('#autoRefresh').change(function () {
    ...
    if (autoRefresh) {
        errorCount = 0;
        refreshLastConsumption();//запись в electricity_consumptions каждую секунду
    };
});
